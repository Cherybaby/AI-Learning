...
本脚本(web_app_attention.py)是「浏览测试集里各个真实样本，对照模型预测与 Adams 真值」的评估页面，
支持拖动滑块切换不同方案(样本)，2x4 曲线网格 + 逐样本 MAE 表。

数据来源：export_test_samples_attention.py 导出的 nn_out_attention/test_samples.json（预先算好，网页只读不重新推理）。
若该文件不存在，启动时会自动导出一次（默认导出全部测试集样本）。

功能：
  - GET  /                    返回对照页面 web/compare.html
  - GET  /api/test_samples    返回 nn_out_attention/test_samples.json 的全部内容
  - GET  /api/metrics         返回 metrics.json 的 test_curve_metrics(逐量MAE/Median/P90/
                              MaxError/D2) + results_worst_samples.json(各量最差Top-N曲线)

启动：
  cd /workspace/Suspension_AI_Design/网页部署
  PYTHONPATH=/workspace/.pylibs TORCH_THREADS=4 python3 web_app_attention.py            # 默认 127.0.0.1:5174
  PYTHONPATH=/workspace/.pylibs TORCH_THREADS=4 python3 web_app_attention.py --port 8080
自测（线程内启动 + 请求，不常驻）：
  PYTHONPATH=/workspace/.pylibs TORCH_THREADS=4 python3 web_app_attention.py --self-test
...
# ------------------------------------------------------------------ #
# 路径与运行环境常量（Attention 模型版本，指向 nn_out_attention/）
# ------------------------------------------------------------------ #
# _HERE：本脚本(web_app_attention.py)所在目录的绝对路径，作为其余相对路径的锚点。
...
_TRAIN_DIR = os.path.join(os.path.dirname(_HERE), "训练推理/Attention")
...
# SAMPLES_PATH：预先导出的测试集样本(预测+真值+MAE)，由 export_test_samples_attention.py 生成。
#               注意 Attention 版本产物目录为 nn_out_attention/(而非 nn_out/)。
SAMPLES_PATH = os.path.join(_TRAIN_DIR, "nn_out_attention", "test_samples.json")
# METRICS_JSON_PATH：整体评估指标，其中 test_curve_metrics 记录逐量 MAE/Median/P90/MaxError/D2。
METRICS_JSON_PATH = os.path.join(_TRAIN_DIR, "nn_out_attention", "metrics.json")
# WORST_SAMPLES_PATH：各运动学量误差最差的 Top-N 曲线，由 visualize_results_attention.py 生成。
WORST_SAMPLES_PATH = os.path.join(_TRAIN_DIR, "nn_out_attention", "results_worst_samples.json")
...
def _ensure_samples():
    """确保 Attention 版本测试集样本(nn_out_attention/test_samples.json)已就绪，并返回解析对象。

    行为：
      1. 若进程内缓存 _CACHE 已填充，直接返回缓存(命中即不再读盘)。
      2. 若磁盘上 SAMPLES_PATH 不存在，则以子进程方式调用「训练推理」目录下的
         export_test_samples_attention.py（--max-samples 0 表示导出全部测试集样本）生成一次；
    ...
    if not os.path.isfile(SAMPLES_PATH):
        print(f"[web2] 未找到 {SAMPLES_PATH}，正在调用 export_test_samples_attention.py 生成…", flush=True)
        r = subprocess.run(
            [sys.executable, os.path.join(_TRAIN_DIR, "export_test_samples_attention.py"),
             "--max-samples", "0"],
    ...
    def do_GET(self):
        """GET 路由分发。各端点说明如下：
          ...
          GET /api/test_samples
              返回 nn_out_attention/test_samples.json 全部内容(application/json)。
